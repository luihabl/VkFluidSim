...
std::vector<glm::vec3> SpawnParticlesInBox(glm::vec3 ll, glm::vec3 ur, u32 count) {
    ...
    for (u32 i = 0; i < count; i++) {
        ...
        float x = ll.x + r1 * (ur.x - ll.x);
        float y = ll.y + r2 * (ur.y - ll.y);
        float z = ll.z + r3 * (ur.z - ll.z);
...
void World3D::UpdateUniforms() {
    global_desc_manager.SetUniformData(&sim_uniform_data);
}

void World3D::InitSimulationPipelines() {
    for (auto& bufs : frame_buffers) {
        bufs = {
            .position_buffer = CreateDataBuffer<glm::vec3>(gfx.GetCoreCtx(), n_particles),
            .velocity_buffer = CreateDataBuffer<glm::vec3>(gfx.GetCoreCtx(), n_particles),
            .density_buffer = CreateDataBuffer<glm::vec2>(gfx.GetCoreCtx(), n_particles),
        };
    }

    predicted_positions = CreateDataBuffer<glm::vec3>(gfx.GetCoreCtx(), n_particles);
    ...
    sort_target_position = CreateDataBuffer<glm::vec3>(gfx.GetCoreCtx(), n_particles);
    sort_target_pred_position = CreateDataBuffer<glm::vec3>(gfx.GetCoreCtx(), n_particles);
    sort_target_velocity = CreateDataBuffer<glm::vec3>(gfx.GetCoreCtx(), n_particles);
...
void World3D::SetBox(glm::vec3 size, glm::vec3 pos) {
    box_pos = pos;
    box_size = size;
}

void World3D::ResetSimulation() {
    auto center = box_pos + box_size / 2.0f;
    auto size = 0.3f * box_size;

    auto spawn_region_ll = box_pos + (box_size - size) / 2.0f;
    auto spawn_region_ur = spawn_region_ll + size;

    auto pos = SpawnParticlesInBox(spawn_region_ll, spawn_region_ur, n_particles);

    for (auto& frame : frame_buffers) {
        SetDataVec(gfx, frame.position_buffer, pos);
        SetDataVal(gfx, frame.velocity_buffer, glm::vec3(0.0f));
        SetDataVal(gfx, frame.density_buffer, glm::vec2(0.0f));
    }
}

void World3D::SetInitialData() {
...
void World3D::HandleEvent(Platform& platform, const SDL_Event& e) {
    ui.HandleEvent(e);
...
void World3D::CopyBuffersToNextFrame(VkCommandBuffer cmd) {
...
void World3D::RunSimulationStep(VkCommandBuffer cmd) {
...
void World3D::Update(Platform& platform) {
    ...
    // auto tr = glm::perspective(70.0f, (float)platform.GetConfig().w /
    // (float)platform.GetConfig().h,
    //                            0.1f, 1000.0f);

    // tr[1][1] *= -1;
    tr = glm::scale(tr, glm::vec3(1 / scale, 1 / scale, 1.0f));

    // tr = glm::rotate(tr, 0.2f, glm::vec3(1.0f, 1.0f, 0.0f));
...
void World3D::DrawUI(VkCommandBuffer cmd) {
    ...
    if (ImGui::CollapsingHeader("Parameters")) {
        glm::vec3 sz = box_size;
        if (ImGui::SliderFloat3("Box size", &sz.x, 0.0f, 20.0f)) {
            SetBox(sz);
            sim_uniform_data.box_size = sz;
